...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Missing: \textbf{epistemic uncertainty}.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		with shared weights.
		\item At test time, classical dropout performs one deterministic forward pass, essentially approximating the ensemble mean.
	\end{itemize}

	\bigskip
	
	Dropout approximation (\alert{Monte Carlo integration}):
	\[
	p(y^*|x^*) \approx
	\frac{1}{T}\sum_{t=1}^T
	p(y^*|x^*,\omega_t)
	...
	If we keep dropout active at test time and sample masks, we approximate the posterior predictive.
...
\begin{frame}
	...
	\begin{itemize}
		...
		q(\omega)
		\]
		\item \alert{Bernoulli} variational family:
		\[
		...
		\]
		\item So Dropout corresponds to \alert{mean field} Bernoulli variational family.
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item So dropout is not just heuristic regularization, it has a \alert{functional Bayesian interpretation}
	\end{itemize}
...
\begin{frame}
	...
	\item $\tau^{-1} I$ captures model precision ($\tau = \frac{pl^2}{2N\lambda}$)
...
\begin{itemize}
		...
		\item BO (Bayesian Optimization) over validation log-likelihood to find optimal $\tau$ (model precision) and $l$ (prior length scale) based on heuristics ($10^{-2}$).
