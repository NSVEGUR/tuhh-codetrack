...
begin
	using PlutoUI,Images,ImageFiltering, CairoMakie, DSP, Interpolations, LinearAlgebra
...
begin
    function preprocessing(img)
        g = Gray.(img)
        g = imfilter(g, Kernel.gaussian(1))
        
        kp_h = centered([-1 0 1; -1 0 1; -1 0 1])
        kp_v = centered([-1 -1 -1; 0 0 0; 1 1 1])
        
        grad_h = imfilter(g, kp_h)
        grad_v = imfilter(g, kp_v)
        
        return sqrt.(grad_h.^2 .+ grad_v.^2)
    end
end

# ╔═╡ eb1650e1-ccf8-4c0a-bde8-7b3454b81b1e
pp_street = preprocessing(street)


# ╔═╡ 4a253e32-1208-40a1-be57-a462fcb13da2
...
begin
    function traffic_light(img)
        ...
        top = img[1:h_div, :]
        mid = img[h_div+1:2*h_div, :]
        bot = img[2*h_div+1:end, :]
        
        m_top = sum(Gray.(top)) / length(top)
        m_mid = sum(Gray.(mid)) / length(mid)
        m_bot = sum(Gray.(bot)) / length(bot)
        
        vals = [m_top, m_mid, m_bot]
        idx = argmax(vals)
...
let
	...
	ax=image(f[1,1],rotr90(cat),axis=(;aspect=1.78))
	hidedecorations!(ax.axis)
	ax=image(f[2,1],rotr90(ikea),axis=(;aspect=1.59))
	hidedecorations!(ax.axis)
	f
end
...
begin
    function replace_green(img)
        green_ref = RGB(0.0, 1.0, 0.0)
        threshold = 45.0
        
        out = copy(RGBA.(img))
        
        for i in eachindex(out)
            if colordiff(img[i], green_ref) < threshold
                out[i] = RGBA(0, 0, 0, 0)
            end
        end
        return out
...
begin
	...
	cat_cut = replace_green(cat)
	ikea_cat = copy(ikea)
	
	for I in CartesianIndices(cat_cut)
	    if checkbounds(Bool, ikea_cat, I)
	        bg = ikea_cat[I]
	        fg = cat_cut[I]
	        a = alpha(fg)
	        ikea_cat[I] = a * RGB(fg) + (1-a) * bg
	    end
	end
end

# ╔═╡ 6ba3b125-a011-49a0-8a63-7fbc14c27ee4
ikea_cat
...
begin
    function replace_color(img, col_old, col_new, threshold)
        out = copy(img)
        for i in eachindex(out)
            if colordiff(img[i], col_old) < threshold
                out[i] = col_new
            end
        end
        return out
    ...
    slj_greenn = replace_color(slj, RGB(1.,1.,1.), RGB(0.,1.,0.), 20.0)
end
...
begin
    function mymagicwand(img, pix, threshold)
        ...
        start_idx = CartesianIndex(pix...)
        start_col = img[start_idx]
        target_col = RGB(0.0, 1.0, 0.0)
        ...
        out[start_idx] = target_col
        
        while !isempty(q)
            curr = popfirst!(q)
            
            for (dr, dc) in [(-1,0), (1,0), (0,-1), (0,1)]
                nr, nc = curr[1]+dr, curr[2]+dc
                
                if 1 <= nr <= rows && 1 <= nc <= cols
                    nidx = CartesianIndex(nr, nc)
                    if !visited[nidx]
                        if colordiff(img[nidx], start_col) < threshold
                            visited[nidx] = true
                            out[nidx] = target_col
                            push!(q, nidx)
                        end
...
# replace with your solution on slj
slj_green2 = mymagicwand(slj, (200, 180), 20.0)
...
begin
    shirt_mask_img = mymagicwand(slj, (200, 180), 20.0)
    
    shirt_transparent = replace_green(shirt_mask_img)
    
    slj_flamed = copy(slj)
    
    for i in eachindex(slj)
        if checkbounds(Bool, flames, i)
            fg = shirt_transparent[i]
            
            if alpha(fg) == 0.0
                slj_flamed[i] = flames[i]
            else
                slj_flamed[i] = slj[i]
            end
        end
...
begin
    function horizontal_fading(img1, img2, pix1, pix2)
        ...
        c1 = min(pix1[2], pix2[2])
        c2 = max(pix1[2], pix2[2])
        dist = c2 - c1
        ...
        for c in 1:w
            ...
            for r in 1:h
                out[r, c] = a * img1[r, c] + (1 - a) * img2[r, c]
            end
        end
        return out
    end
...
begin
    function vertical_fading(img1, img2, pix1, pix2)
        ...
        r1 = min(pix1[1], pix2[1])
        r2 = max(pix1[1], pix2[1])
        dist = r2 - r1
...
md"
###### 🎓🎓 g)
**Write a function `myfading(img1,img2,pix1,pix2)`, which takes two images of the same size and two pixels. The angle and size of the fading is determined by the line between `pix1` and `pix2`. Between `pix1` and `pix2` the image should have a fading from `img1` to `img2` over the whole image (in normal direction to the line). The fading can be linear or use a sigmoid-function. Use your solution to** 
	
**(1) fade the images `vader` and `anakin` between `(75,60)` and `(225,180)` and save the resulting image as `vanakin`, and** 
	
**(2) fade the images `anakin` and `vader` between `(75,180)` and `(225,60)` and save the resulting image as `avader`.**
"
...
begin
    function myfading(img1, img2, pix1, pix2)
        ...
        for r in 1:h
            for c in 1:w
                p = [r, c]
                proj = dot(p - p1, vec) / len_sq
                a = 1.0 - clamp(proj, 0.0, 1.0)
                out[r, c] = a * img1[r, c] + (1 - a) * img2[r, c]
            end
        end
        return out
...
begin
	# replace with your solution..
    vanakin = myfading(vader, anakin, (75,60), (225,180))
end

# ╔═╡ d480b301-3a20-4455-93bd-fa16e86e0eed
begin
avaderr = myfading(anakin, vader, (75,180), (225,60))
end
...
manifest_format = "2.0"
project_hash = "b647b1c7983a2545627c3269def3f386a8eb5c80"
