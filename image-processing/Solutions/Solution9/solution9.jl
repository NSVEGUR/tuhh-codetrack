...
$\begin{align}
	(A\circ B) = (A\ominus B)\oplus B = \hspace{-0.8cm}\bigcup\limits_{\hspace{0.8cm} b \in \{b\in I_n|B_b \subset A\}} \hspace{-1cm} B_b \hspace{0.5cm} \subset A
\end{align}$
...
$\begin{align}
	(A\circ B)\circ B = \hspace{-0.2cm}\bigcup\limits_{\hspace{0.2cm}B_b \subset (A\circ B)}\hspace{-0.3cm} B_b = \hspace{-0.2cm}\bigcup\limits_{\hspace{0.2cm}B_b \subset \bigcup\limits_{B_b \subset A} \hspace{-0.1cm} B_b} \hspace{-0.3cm} B_b = \bigcup\limits_{B_b \subset A} B_b = (A\circ B)
\end{align}$
...
$\begin{align}
	(C\circ B) = \bigcup\limits_{B_b \subset C} B_b \subset \bigcup\limits_{B_b \subset D} B_b = (D\circ B)
\end{align}$
...
$\begin{align}
	(C \bullet B) = \bigcap\limits_{b \in B} \left(\bigcup\limits_{b \in B} C_{b}\right)_{-b} \subset \bigcap\limits_{b \in B} \left(\bigcup\limits_{b \in B} D_{b}\right)_{-b} = (D\bullet B)
\end{align}$
The union of all translated D's is a subset of the union of all translated C's. The intersection with the same translation backwards can not increase the set.
"""
...
function mybarcodescanner(img)
	...
	# Step 1: convert to grayscaled image
	img_g = Gray.(img)

	# Step 2: get the gradient-images using a Sobel-filter in x and in y direction and
	#         substract the absolute values of the y-gradient from the absolute value 
	#         of the x gradient
	Gy = [1 2 1;0 0 0;-1 -2 -1]
	Gx = Gy'
	img_grad = abs.(imfilter(img_g,Gx))-abs.(imfilter(img_g,Gy))

	# Step 3: blurr the image with a 9x9 gaussian kernel (Kernel.gaussian(2))
	img_blurred = imfilter(img_grad,Kernel.gaussian(2))

	# Step 4: binarize the image using a threshold-value t=0.7
	img_bin = (x->x<=0.7 ? 0 : 1).(img_blurred)

	# Step 5: Use the closing-operator with a rectangular kernel of size (7,21) to 
	#         fill the gaps between the barcode-stripes
	closed = Gray.(myclosing(img_bin, ones(Bool,7,21)))

	# Step 6: Use the erosion-operator four times and after that the
	#         dilation-operator four times to get rid of bigger noise-blobs
	closed_3 = mydilate(mydilate(mydilate(mydilate(myerode(myerode(myerode(myerode(closed))))))))

	# Step 7: Use the above given function find_contours() to get an array of 
	#         index-sets. Each index-set in this array describes a connected area of 
	#         ones in the binarized image. Sort the index-sets by there sizes and 
	#         select the biggest one as the barcode-area
	contours = find_contours(closed_3)
	ind = contours[sortperm(size.(contours))[end]]

	# Step 8: Use the above given function boundingbox() to get a minimum rectangular
	#         bounding-box around the barcode area and set the color of the 
	#         output-file at the indices of the bounding-box to green
	output[boundingbox(ind)].=RGB(0,1,0)
...
# ╔═╡ 1b2b318d-c83d-464f-b003-0946f771204d
md"""
...
# ╔═╡ 56f0b658-1ff6-4957-9fcd-984506a7187b
begin
function myzebradetector(img)
	...
	Gy = [1 2 1;0 0 0;-1 -2 -1]
	Gx = Gy'
	img_grad = abs.(imfilter(img_g,Gy))+abs.(imfilter(img_g,Gx))
	
	img_blurred = imfilter(img_grad,Kernel.gaussian(0.5))
	
	img_bin = (x->x<=0.88 ? 0 : 1).(img_blurred)
	
	closed = Gray.(myclosing(img_bin, ones(Bool,3,3)))
...
function myerode_g(A, se=ones(Bool,3,3))
	...
	N,M = floor.(Int,size(se)./2)
	se = OffsetArray(se,CartesianIndex(-N,-M):CartesianIndex(N,M))
	out = copy(A)
	for r in CartesianIndices(A)
		out[r] = minimum([(z+r) in CartesianIndices(A) ? A[r+z] : Inf for z in CartesianIndices(se)])
	end
	return out
end
...
function mydilate_g(A, se=ones(Bool,3,3))
	...
	out = copy(A)
	for r in CartesianIndices(A)
		out[r] = maximum([(r-z) in CartesianIndices(A) ? A[r-z] : 0 for z in CartesianIndices(se)])
	end
	return out
end
...
myopening_g(A, args...) = mydilate_g(myerode_g(A,args...),args...)
...
myclosing_g(A, args...) = myerode_g(mydilate_g(A,args...),args...)
...
mytophat(A, args...) = A - myopening_g(A, args...)
...
mybottomhat(A, args...) = myclosing_g(A, args...) - A
...
###### 🎓 g)
**Write a function `myspeedcamera(A)`, which takes an image of a car `A` and highlights the numbers on the license plate using top hat and bottom hat operator and a kernel of size `(5,13)`. Test your function on the image `car`.**
"
...
function myspeedcamera(A;size=(5,13))
	A_g=Gray.(A)
	(n,m)=size
	return mybottomhat(mytophat(A_g, ones(Bool,n,m)), ones(Bool,n,m))
end
...
let x=[ 0.341176  0.117647  0.117647  0.117647   0.145098   0.572549
 ...
 0.164706  0.164706  0.227451  0.0941176  0.0941176  0.0941176]
	if !(isapprox(norm(Float64.(myerode_g(Gray.(testimg))).-x),0,atol=1e-3))
...
let x=[ 0.666667  0.666667  0.815686  0.815686  0.815686  0.772549
 ...
 0.568627  0.729412  0.729412  0.729412  0.623529  0.545098]
	if !(isapprox(norm(Float64.(mydilate_g(Gray.(testimg))).-x),0,atol=1e-3))
...
let x=[ 0.341176  0.341176  0.117647  0.145098  0.572549   0.572549
 ...
 0.164706  0.227451  0.227451  0.227451  0.0941176  0.0941176]
	if !(isapprox(norm(Float64.(myopening_g(Gray.(testimg))).-x),0,atol=1e-3))
...
let x=[0.666667  0.666667  0.666667  0.815686  0.772549  0.772549
 ...
 0.568627  0.568627  0.729412  0.623529  0.545098  0.545098]
	if !(isapprox(norm(Float64.(myclosing_g(Gray.(testimg))).-x),0,atol=1e-3))
...
# ╔═╡ 4edf90f2-2696-4be3-b35b-c051e838acf1
let x=[0.027451  0.0       0.537255  0.0       0.2        0.0
 ...
 0.239216  0.0       0.501961  0.372549  0.0        0.34902]
	if !(isapprox(norm(Float64.(mytophat(Gray.(testimg))).-x),0,atol=1e-3))
...
let x=[0.298039  0.32549    0.0117647  0.670588   0.0       0.2
 ...
 0.164706  0.341176   0.0        0.0235294  0.45098   0.101961]
	if !(isapprox(norm(Float64.(mybottomhat(Gray.(testimg))).-x),0,atol=1e-3))
...
let x=[ 0.447059   0.47451   0.160784   0.670588  0.0431373  0.243137
 ...
 0.32549    0.501961  0.0        0.129412  0.635294   0.286275]
	if !(isapprox(norm(Float64.(myspeedcamera(Gray.(testimg))).-x),0,atol=1e-3))
