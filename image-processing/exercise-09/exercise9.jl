...
function mybarcodescanner(img)
	...
	grad_x = imfilter(gray, sobel_x)
	grad_y = imfilter(gray, sobel_y)
	gradient = abs.(grad_x) .- abs.(grad_y)
	blurred = imfilter(gradient, Kernel.gaussian(1.5))
	threshold = 0.7
	binary = blurred .> threshold
	se_close = ones(Bool, 7, 21)
	closed = myclosing(binary, se_close)
	cleaned = closed
	for _ in 1:4
        cleaned = myerode(cleaned)
    end
    for _ in 1:4
        cleaned = mydilate(cleaned)
    end
	contours = find_contours(cleaned)
	if isempty(contours) return img end
	sort!(contours, by = length, rev = true)
    barcode_area = contours[1]
	bbox_inds = boundingbox(barcode_area)
	output = copy(img)
    green = RGB(0, 1, 0)
	for I in bbox_inds
	    output[I] = green
...
function myzebradetector(img)
    gray = Gray.(img)
    grad_x = imfilter(gray, [-1 0 1; -2 0 2; -1 0 1])
    grad_y = imfilter(gray, [-1 -2 -1; 0 0 0; 1 2 1])
    gradient = abs.(grad_x) .+ abs.(grad_y)
    blurred = imfilter(gradient, Kernel.gaussian(1.5))
	threshold = 0.7
    binary = blurred .> threshold
    se_close = ones(Bool, 21, 7)
    closed = myclosing(binary, se_close)
    cleaned = closed
	for _ in 1:12
        cleaned = myerode(cleaned)
    ...
    if isempty(contours) return img end
    sort!(contours, by = length, rev = true)
    zebra_area = contours[1] 
    bbox_inds = boundingbox(zebra_area)
    output = copy(img)
    green = RGB(0, 1, 0)
    for I in bbox_inds
        output[I] = green
...
function myerode_g(A, se=ones(Bool,3,3))
    ...
    for j in 1:cols
        for i in 1:rows
            ...
            # Manual loop over the window
            for wj in -rw:rw
                for wi in -rh:rh
                    # Clamp indices to stay inside image (padding)
                    ci = clamp(i + wi, 1, rows)
                    cj = clamp(j + wj, 1, cols)
                    
                    min_val = min(min_val, A[ci, cj])
                end
            end
            out[i, j] = min_val
        end
...
function myopening_g(A, se=ones(Bool,3,3))
	output = mydilate_g(myerode_g(A, se), se)
	return output
end
...
function myclosing_g(A, se=ones(Bool,3,3))
	output = myerode_g(mydilate_g(A, se), se)
	return output
end
...
function myspeedcamera(A;size=(5,13))
	gray = Gray.(A)
    se = ones(Bool, size...)
    plate_region = mytophat(gray, se)
    numbers = mybottomhat(plate_region, se)
	return Gray.(numbers)
end
...
manifest_format = "2.0"
project_hash = "62356833df2418452da731dd240ce54ddab07afc"
