...
from amllib.losses import Loss, CrossEntropy
...
class Sequential:
    ...
    def __init__(self, input_shape: tuple[int, ...],
                 layers: list[Layer] = None, loss: Loss = None):
        """
        ...
        loss : Loss
            Loss for the training of the network. Default value: a new
            CrossEntropyLoss instance assuming SoftMax activation in the
            last layer.
        """
    ...
    def backprop(self, input: np.ndarray, labels: np.ndarray) -> None:
        ...
        if input.shape[1:] != self.input_shape:
            raise ValueError("The input shape does not match the output shape"
                             f"of the network! Shape {self.input_shape} was"
                             f" expected, but it was {input.shape[1:]}")

        if labels.shape[1:] != self.output_shape:
            raise ValueError("The labels shape does not match the output shape"
                             f"of the network! Shape {self.output_shape} was "
                             f"expected, but it was {labels.shape[1:]}")

        # TODO Implement the backpropagation through all layers.
        # Compute the loss and the derivative of the loss function by the output of the network
        # as intial input for the backpropagation method of the last layer.
        # This can be done with the `backprop` method of the loss function of
        # the network. Then propagate backwards through all layers.
        # The loss can be computed with the loss function set in __init__.
        # The derivative of the loss function by the output of the network
        # can be computed with the backprop method of the loss function.
        a_out = self.feedforward(input)
        loss = self.loss(a_out, labels)
        n_batch, _ = input.shape
        delta = self.loss.backprop(a_out, labels) / n_batch
    ...
    def train(self, train_input: np.ndarray, train_labels: np.ndarray,
        ...
        for e in range(epochs):
            ...
            for j in range(n_batches):
                ...
                batch_losses[j] = self.backprop(
                        train_input[p[j*batch_size:(j+1)*batch_size], :],
    ...
    def add_layer(self, layer: Layer):
        ...
        layer : Layer
            Layer to add.
        """
        # Build the layer with the old output shape of the network, if
        # not done yet.
        if not layer.built:
