...
class FeedforwardNet:
    ...
    def feedforward(self, x: np.ndarray) -> np.ndarray:
        ...
        self.__a[0] = x
        for i, (W, b, afun) in enumerate(zip(self.weights, self.biases, self.afuns)):
            self.__z[i] = W @ self.__a[i] + b
            self.__a[i+1] = afun.feedforward(self.__z[i])
        y = self.__a[-1]
        return y
    ...
    def backprop(self, x: np.ndarray, y: np.ndarray) -> None:
        ...
        for i, (W, afun, a) in enumerate(zip(reversed(self.weights), reversed(self.afuns), reversed(self.__a[:-1]))):

            delta = afun.backprop(delta)

            self.dW[-i-1] = delta @ a.T
            self.db[-i-1] = np.sum(delta, axis=1, keepdims=True)
